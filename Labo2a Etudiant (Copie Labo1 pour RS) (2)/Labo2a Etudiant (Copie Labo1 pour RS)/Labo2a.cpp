...
//DESCRIPTION: Echange de deux unites selectionnees aleatoirement.
void EchangeDeuxUnites(TSolution& Voisin, TProblem unProb, TAlgo& unAlgo);
...
int main(int NbParam, char *Param[])
{
	...
	do
	{
		...
		if (LAlgo.Delta <= 0)	//**deplacement amelioration ou egalite
		{
			Courante = Next;
...
TSolution AppliquerVoisinage(const TSolution uneSol, TProblem unProb, TAlgo& unAlgo)
{
	...
	EchangeDeuxUnites(Copie, unProb, unAlgo);
...
void EchangeDeuxUnites(TSolution& Voisin, TProblem unProb, TAlgo& unAlgo)
{
	...
	//Echange des 2 unites
	swap(Voisin.Site[PosA], Voisin.Site[PosB]); //Echange de 2 elements dans un vecteur (fonction dans la biblio functional)
}